...
| `browserVersion` | Specify which version of Google Chrome for Testing will be used to run. The default value is `default`. `default` will run the version pre-installed in the Docker image. `latest` will use the latest version available from the Google Chrome Labs [Known Good Versions](https://googlechromelabs.github.io/chrome-for-testing/known-good-versions.json). `xxx.x.xxxx.x` will use this specific version to run. Chrome 116 is the earliest supported version. | Optional |
